...
p "A macro cannot solve anything that cannot be solved with Base SAS code. But it can reduce the effort needed to write that code, and it can help in making code data-driven. Also see Maxim 33 in reference to call execute.";
%end;
%let maxim=%eval(&maxim. +1);
%if &maxim. in (&maxims.) or &ALL. %then %do;
p "";
...
p "As long as you keep your ability and will to learn, you are alive. When you stop learning, you may not be dead, but you start smelling funny. Never say 'I do not have the time to learn that now'.";
...
p "Never restrict yourself by a simple 'Use XYZ' or 'Do not use XYZ'. If something is better solved with a certain procedure, use it. If you do not yet know it, learn it (see Maxim 13).";
...
p "Make sure to gain knowledge about the environment in which SAS is implemented. Know the system's layout, its basic syntax, and its most important utilities. Especially UNIX is rich with tools that can and will make your life easier. Control the system, do not let the system control you.";
%end;
%let maxim=%eval(&maxim. +1);
%if &maxim. in (&maxims.) or &ALL. %then %do;
p "";
h3 "Maxim 16: If It Is Not Written, It Is Not." ;
...
p "(Do not keep data in structure)";
...
p "Store your code in .sas files, use Enterprise Guide projects as temporary containers only while developing. Text lends itself well to versioning and other tools that help the programmer, binary files do not. See Maxim 26.";
%end;
%let maxim=%eval(&maxim. +1);
%if &maxim. in (&maxims.) or &ALL. %then %do;
p "";
h3 "Maxim 28: Macro Variables Need No Formats." ;
p "Keep in mind that '01jan1960'd and 0 represent the same value in SAS code. Therefore you only need formatting of macro variables when they are meant for display (e.g., in a title statement). ";
...
p "Even use COMPARE at crucial steps in your development process, so you catch mistakes when they are introduced, instead of having to search for that spot later. If COMPARE takes its time, enjoy your coffee. It is time well spent.";
%end;
%let maxim=%eval(&maxim. +1);
%if &maxim. in (&maxims.) or &ALL. %then %do;
p "";
h3 "Maxim 33: Intelligent Data Makes for Intelligent Programs." ;
p 'The Jedi programmer strives for intelligent data structures. The code will then be intelligent and well-designed (almost) on its own.';
p '"Bad programmers worry about the code. Good programmers worry about data structures and their relationships." (Linus Torvalds)';
...
p "Do not solve a task in one everything-including-the-kitchen-sink step. Create intermediate datasets/results, so that each step solves one issue at a time that can easily be comprehended (see Maxim 29). ";
p "In the same vein, do not implement multiple changes to a program in one iteration. Instead do a check-in for every single logical change. ";
...
p "#1 This is old, and therefore it is good"/ style=[font_weight=bold];
p "#2 This is new, and therefore it is better."/ style=[font_weight=bold];
...
p '(e.g., make a batch program crash on invalid data, so the scheduler catches it). (With regards to Larry Niven.)';
%end;
%let maxim=%eval(&maxim. +1);
%if &maxim. in (&maxims.) or &ALL. %then %do;
p "";
...
p "Make it a habit to always terminate macro variable references with a dot. It is never wrong to have the dot, but omitting it can have consequences and cause confusion.";
p "See:";
...
p "The rather recently (version 9) introduced hash object provides a high-performance tool for in-memory lookups that helps solve complicated issues which required complex programming earlier on, or multi-step operations consuming time and disk space.Learning its basic operation, and use in advanced situations is a must for the aspiring SAS programmer.A very good introduction is found in SAS(R) Hash Object Programming Made Easy by Michele M. Burlew";
%end;
%let maxim=%eval(&maxim. +1);
%if &maxim. in (&maxims.) or &ALL. %then %do;
p "";
